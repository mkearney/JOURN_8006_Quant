...
ggplot(df, aes(x, (1 - y) + .3)) + 
  ...
  theme_mwk(light = "transparent") + 
  theme(axis.title = element_blank(), axis.text = element_blank(),
    plot.margin = margin(0, -45, -4, -48)) + 
  annotate("text", -1.5, .4, label = "JOURN 8006", size = rel(28), 
    fontface = "bold", family = "Roboto Condensed", alpha = .8) + 
  scale_x_continuous(breaks = seq(-3, 3, .25)) + 
  scale_y_continuous(breaks = seq(-.1, 1.4, .25), limits = c(-.1, 1.4)) + 
  ggsave("~/Desktop/journ8006-canvas-banner.png", width = 12, 
